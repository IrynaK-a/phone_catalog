...
export const CategoryCard: React.FC<Props> = ({
  ...
  const modelsCount = allProducts.filter(
    product => product.category === pass,
  ).length;

  return (
    <Link to={pass} className="category-card" data-cy="categoryLinksContainer">
      <img
        src={`./img/category-${pass}.webp`}
        alt={title}
      ...
      <div className="category-card__text-container">
        ...
        <p className="category-card__models-count">{`${modelsCount} models`}</p>
      </div>
    </Link>
  );
};
